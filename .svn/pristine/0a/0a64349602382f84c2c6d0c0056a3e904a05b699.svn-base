...
$(function () {
   ...
   //  alert("测试有用功率");
    $("#show_Line_btn").click(function () {
        ...
        // alert(endtime);
        var url = contextpath + "/activePower/lineShowByTimeSpace.power";
        // alert(url);
        var data = {};
        ...
        $.post(url,data,function (result,status) {
            ...
            if(status=="success"){//"success".equals(status)
                ...
                var paList = result.paList;
                var pbList = result.pbList;
                var pcList = result.pcList;
                var ptList = result.ptList;
...
// 为echarts对象加载数据
                    ...
                    alert("没有此段时间的数据！");
                    window.location.href = contextpath + "/activePower/showAllActivePowerInfo.power";
                }
            }
        },"json");
    ...
    $("#show_table_btn").click(function () {
        ...
        // alert(starttime);
        contextpath = $("#contextpath").val();
        var url = contextpath + "/activePower/tableShowByTimeSpace.power";
        var data = {};
        data.starttime = starttime;
        data.endtime = endtime;
        $.ajaxSetup({
                ...
                if (result.realTimeDatasList.length>0 ){
                    ...
                    for (var i = 0;i<result.realTimeDatasList.length;i++){
                        newtable += "<tr>" +
                            ...
                            "<td>"+result.realTimeDatasList[i].pa+"</td>" +
                            "<td>"+result.realTimeDatasList[i].pb+"</td>" +
                            "<td>"+result.realTimeDatasList[i].pc+"</td>" +
                            "<td>"+result.realTimeDatasList[i].pt+"</td>" +
                            "</tr>";
                    }
