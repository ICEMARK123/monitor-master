...
$(function () {
    ...
    $("#show_Line_btn").click(function () {
        ...
        // alert(endtime);
        var url = contextpath + "/reactivePower/lineShowByTimeSpace.power";
        // alert(url);
        var data = {};
        ...
        $.post(url,data,function (result,status) {
            ...
            if(status=="success"){//"success".equals(status)
                ...
                var QbList = result.QbList;
                var QcList = result.QcList;
                var QtList = result.QtList;
...
// 为echarts对象加载数据
                    ...
                    alert("没有此段时间的数据！");
                    window.location.href = contextpath + "/reactivePower/showAllActivePowerInfo.power";
                }
            }
        },"json");
    ...
    $("#show_table_btn").click(function () {
        ...
        // alert(starttime);
        contextpath = $("#contextpath").val();
        var url = contextpath + "/reactivePower/tableShowByTimeSpace.power";
        var data = {};
        data.starttime = starttime;
        data.endtime = endtime;
        $.ajaxSetup({
                ...
                if (result.realTimeDatasList.length>0 ){
                    ...
                    for (var i = 0;i<result.realTimeDatasList.length;i++){
                        newtable += "<tr>" +
                            ...
                            "<td>"+result.realTimeDatasList[i].qa+"</td>" +
                            "<td>"+result.realTimeDatasList[i].qb+"</td>" +
                            "<td>"+result.realTimeDatasList[i].qc+"</td>" +
                            "<td>"+result.realTimeDatasList[i].qt+"</td>" +
                            "</tr>";
                    }
